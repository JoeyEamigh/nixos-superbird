...
if [ ! -f "$root"/qemu/kernel ]; then
  nix build '.#nixosConfigurations.qemu-example.config.system.build.toplevel' --show-trace -j"$(nproc)"
  cp result/kernel "$root"/qemu
fi

if [ ! -f "$root"/qemu/btrfs.img ]; then
  nix build '.#nixosConfigurations.qemu-example.config.system.build.btrfs' --show-trace -j"$(nproc)"
  cp result "$root"/qemu/btrfs.img
fi

if [ ! -f "$root"/qemu/initrd.img ]; then
  nix build '.#nixosConfigurations.qemu-example.config.system.build.initrd' --show-trace -j"$(nproc)"
